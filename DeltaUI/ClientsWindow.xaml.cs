...
namespace DeltaUI
{
	...
	{
		public string DeltaVersion = "1.0.5";
		...
		[DllImport("Delta.dll", CallingConvention = CallingConvention.Cdecl)]
		private static extern void Initialize();

		[DllImport("Delta.dll", CallingConvention = CallingConvention.Cdecl)]
		private static extern IntPtr GetClients();

		[DllImport("Delta.dll", CallingConvention = CallingConvention.Cdecl, CharSet = CharSet.Ansi)]
		...
		private async void LoadSupportedVersion()
		{
			try
			{
				...
				string latestVersion = await client.GetStringAsync;
				if (latestVersion != DeltaVersion)
				{
					MessageBox.Show($"The current version {DeltaVersion} is outdated.\n\nPlease download the latest version of Delta", "Outdated Delta version", MessageBoxButton.OK, MessageBoxImage.Warning);
					Application.Current.Shutdown();
				}
				SupportedVersion = await client.GetStringAsync;
			}
		...
		private async Task AddClientCheckBox(ClientInfo client)
		{
			...
			{
				MessageBox.Show($"Delta might not be compatible on the client {client.name} with {client.version}\n\nSupported version: {SupportedVersion}\n\nClick OK to continue using Delta.", "Version Mismatch", MessageBoxButton.OK, MessageBoxImage.Warning);
			}
		}
