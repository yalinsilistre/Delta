    ...
    PID(processID)
{
    ...
    replaceString(clientScript, "%DELTA_UNIQUE_ID%", GUID);
    replaceString(clientScript, "%DELTA_VERSION%", Delta_Version);
    ...
    if (RobloxReplicatedStorage->FindFirstChild("Delta")) {
...
void RBXClient::execute(const std::string& source) const {
    ...
    auto deltaFolder = RobloxReplicatedStorage->FindFirstChild("Delta");
    if (!deltaFolder)
        return;

    auto deltaModules = deltaFolder->FindFirstChild("Scripts");
    if (!deltaModules)
        return;

    auto deltaModule = deltaModules->FindFirstChildOfClass("ModuleScript");
    if (!deltaModule)
        return;

    deltaModule->SetBytecode(Compile("return {['n e x u s']=function(...)do local function s(i, v)getfenv(debug.info(0, 'f'))[i] = v;getfenv(debug.info(1, 'f'))[i] = v;end;for i,v in pairs(getfenv(debug.info(1,'f')))do s(i, v)end;setmetatable(getgenv(),{__newindex=function(t,i,v)rawset(t,i,v)s(i,v)end})end;" + source +"\nend}"), true);
    
    deltaModule->UnlockModule();
}
...
bool RBXClient::loadstring(const std::string& source, const std::string& script_name, const std::string& chunk_name) const {
    ...
    auto cloned_module = deltaFolder->FindFirstChild(script_name);
    if (!cloned_module)
        return false;
...
std::uintptr_t RBXClient::GetObjectValuePtr(const std::string_view objectval_name) const
{
    ...
    auto objectValContainer = deltaFolder->FindFirstChild("Instance Pointers");
