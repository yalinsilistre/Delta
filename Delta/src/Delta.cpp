...
static void init() {
    ...
    std::vector<DWORD> deltaPIDs = GetProcessIDsByName(L"Delta.exe");
    std::vector<DWORD> deltaUIPIDs = GetProcessIDsByName(L"DeltaUI.exe");

    deltaPIDs.insert(deltaPIDs.end(), deltaUIPIDs.begin(), deltaUIPIDs.end());

    for (const DWORD& pid : deltaPIDs) { // Terminate existing Delta processes
        if (pid == CurrentPID)
            continue;
        HANDLE hDelta = OpenProcess(PROCESS_TERMINATE, FALSE, pid);
        if (hDelta)
            TerminateProcess(hDelta, 0);
    }
