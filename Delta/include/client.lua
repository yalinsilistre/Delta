--!native
--!optimize 2
local DELTA_UNIQUE = "%DELTA_UNIQUE_ID%"
...
if RobloxReplicatedStorage:FindFirstChild("Delta") then return end

local DeltaContainer = Instance.new("Folder", RobloxReplicatedStorage)
DeltaContainer.Name = "Delta"
local objectPointerContainer, scriptsContainer = Instance.new("Folder", DeltaContainer), Instance.new("Folder", DeltaContainer)
...
local Delta = {
	about = {
		_name = 'Delta',
		_version = '%DELTA_VERSION%',
		_publisher = "risazaza"
	}
}
table.freeze(Delta.about)
...
function Bridge:InternalRequest(body, timeout)
	...
	if success and result then
		error("DELTA SERVER ERROR: " .. tostring(result), 2)
	end
...
function Bridge:loadstring(source, chunkName)
	...
	coreModule.Parent = DeltaContainer
	...
	if result then
		...
		while task.wait() do
			...
			if (tick() - clock > 5) then
				warn("[DELTA]: loadstring failed and timed out")
				for _, module in pairs(cachedModules) do
					module:Destroy()
				end
...
function Bridge:request(options)
	...
	return {
		Success = false,
		StatusMessage = "Can't connect to Delta web server: " .. self.serverUrl,
		StatusCode = 599;
...
Delta.Delta = {
	...
	HttpSpy = function(state)
		...
		Delta.rconsoleinfo("Http Spy is set to '" .. tostring(state) .. "'")
		httpSpy = state
	end,
}

function Delta.Delta.get_real_address(instance)
...
function Delta.Delta.spoof_instance(instance, newinstance)
	...
	do
		if type(newinstance) == "number" then 
			...
			newAddress = Delta.Delta.get_real_address(newinstance)
		end
	end
	local objectValue = Instance.new("ObjectValue", objectPointerContainer)
	objectValue.Name = HttpService:GenerateGUID(false)
...
function Delta.Delta.GetGlobal(global_name)
...
function Delta.Delta.SetGlobal(global_name, value)
...
function Delta.Delta.Compile(source)
...
function Delta.require(moduleScript)
...
function Delta.loadstring(source, chunkName)
...
function Delta.request(options)
	...
	if httpSpy then
		Delta.rconsoleprint("-----------------[Delta Http Spy]---------------\nUrl: " .. options.Url .. 
	...
	options.Headers["User-Agent"] = options.Headers["User-Agent"] or "Delta/RobloxApp/" .. tostring(Delta.about._version)
	options.Headers["Exploit-Guid"] = tostring(hwid)
	options.Headers["Delta-Fingerprint"] = tostring(hwid)
...
Delta.http = {request = Delta.request}
Delta.http_request = Delta.request

function Delta.HttpGet(url, returnRaw)
	assert(type(url) == "string", "invalid argument #1 to 'HttpGet' (string expected, got " .. type(url) .. ") ", 2)
	local returnRaw = returnRaw or true

	local result = Delta.request({
		Url = url,
		Method = "GET"
	})
...
function Delta.HttpPost(url, body, contentType)
...
function Delta.GetObjects(asset)
	return {
...
Delta.Game = Delta.game

Delta.workspace = newproxy(true)
local workspaceProxy = getmetatable(Delta.workspace)
...
Delta.Workspace = Delta.workspace

function Delta.getgenv()
	return Delta
end
...
function Delta.writefile(path, content)
...
function Delta.appendfile(path, content)
	...
	Delta.writefile(path, readVal .. content)
end
function Delta.loadfile(path)
	assert(type(path) == "string", "invalid argument #1 to 'loadfile' (string expected, got " .. type(path) .. ") ", 2)
	return Delta.loadstring(Delta.readfile(path))
end
Delta.dofile = Delta.loadfile
function Delta.isfolder(path)
...
function Delta.isfile(path) -- return not Delta.isfolder(path)
...
function Delta.makefolder(path)
...
function Delta.getcustomasset(path)
...
Delta.base64 = base64
Delta.base64_encode = base64.encode
Delta.base64encode = base64.encode
Delta.base64_decode = base64.decode
Delta.base64decode = base64.decode
...
Delta.crypt.generatebytes = function(len)
	return Delta.crypt.generatekey(len)
end
Delta.crypt.random = function(len)
	return Delta.crypt.generatekey(len)
end
Delta.crypt.decrypt = Delta.crypt.encrypt

function Delta.crypt.hash(txt, hashName)
...
Delta.hash = Delta.crypt.hash

Delta.crypt.lz4 = lz4
Delta.crypt.lz4compress = lz4.compress
Delta.crypt.lz4decompress = lz4.decompress

Delta.lz4 = lz4
Delta.lz4compress = lz4.compress
Delta.lz4decompress = lz4.decompress

local Drawing, drawingFunctions = DrawingLib.Drawing, DrawingLib.functions
Delta.Drawing = Drawing
...
-- / Miscellaneous \ --
function Delta.getproperties(instance)
...
Delta.gethiddenproperties = Delta.getproperties

local _saveinstance = nil
function Delta.saveinstance(options)
	options = options or {}
	...
	_saveinstance = _saveinstance or Delta.loadstring(Delta.HttpGet("https://raw.githubusercontent.com/luau/SynSaveInstance/main/saveinstance.luau", true), "saveinstance")()
	return _saveinstance(options)
end
Delta.savegame = Delta.saveinstance

function Delta.getexecutorname()
	return Delta.about._name
end
function Delta.getexecutorversion()
	return Delta.about._version
end

function Delta.identifyexecutor()
	return Delta.getexecutorname(), Delta.getexecutorversion()
end
Delta.whatexecutor = Delta.identifyexecutor

function Delta.get_hwid()
	return hwid
end
Delta.gethwid = Delta.get_hwid

function Delta.getscriptbytecode(script_instance)
...
Delta.dumpstring = Delta.getscriptbytecode
...
local function konst_call(konstantType: string, scriptPath: Script | ModuleScript | LocalScript): string
	local success: boolean, bytecode: string = pcall(Delta.getscriptbytecode, scriptPath)
	...
	local httpResult = Delta.request({
...
function Delta.Decompile(script_instance)
...
Delta.__disassemble = Delta.__Disassemble

function Delta.queue_on_teleport(source)
...
Delta.queueonteleport = Delta.queue_on_teleport

function Delta.setclipboard(content)
...
Delta.toclipboard = Delta.setclipboard

function Delta.rconsoleclear()
	return Bridge:rconsole("cls")
end
Delta.consoleclear = Delta.rconsoleclear

function Delta.rconsolecreate()
	return Bridge:rconsole("crt")
end
Delta.consolecreate = Delta.rconsolecreate

function Delta.rconsoledestroy()
	return Bridge:rconsole("dst")
end
Delta.consoledestroy = Delta.rconsoledestroy

function Delta.rconsoleprint(...)
	local text = ""
	for _, v in {...} do
...
Delta.consoleprint = Delta.rconsoleprint

function Delta.rconsoleinfo(...)
	local text = ""
	for _, v in {...} do
		text = text .. tostring(v) .. " "
	end
...
Delta.consoleinfo = Delta.rconsoleinfo

function Delta.rconsolewarn(...)
	local text = ""
	for _, v in {...} do
		text = text .. tostring(v) .. " "
	end
...
Delta.consolewarn = Delta.rconsolewarn

function Delta.rconsolesettitle(text)
...
Delta.rconsolename = Delta.rconsolesettitle
Delta.consolesettitle = Delta.rconsolesettitle
Delta.consolename = Delta.rconsolesettitle

function Delta.clonefunction(func)
...
function Delta.islclosure(func)
...
function Delta.fireclickdetector(part)
...
function Delta.firetouchinterest(toucher, toTouch, touch_state)
	...
	local toTouchAddress = tostring(Delta.Delta.get_real_address(toTouch))
	...
	if touch_state == 0 then
		...
		Delta.Delta.spoof_instance(newPart, toTouch)
	...
	elseif touch_state == 1 then
		...
		Delta.Delta.spoof_instance(toTouch, tonumber(toTouchAddress))
...
function Delta.fireproximityprompt(proximityprompt, amount, skip)
...
function Delta.setsimulationradius(newRadius, newMaxRadius)
...
function Delta.rconsoleinput(text)
	task.wait()
	return "N/A"
end
Delta.consoleinput = Delta.rconsoleinput
...
local renv = {
	...
	game = Delta.game, workspace = Delta.workspace, Game = Delta.game, Workspace = Delta.workspace,

	getmetatable = getmetatable, setmetatable = setmetatable
}
table.freeze(renv)

function Delta.getrenv()
	return renv
end

function Delta.isexecutorclosure(func)
	assert(type(func) == "function", "invalid argument #1 to 'isexecutorclosure' (function expected, got " .. type(func) .. ") ", 2)
	for _, genv in Delta.getgenv() do
		if genv == func then
			return true
		end
	end
	...
	if check(Delta.getgenv().getrenv()) then
		return false
	end
	return true
end
Delta.checkclosure = Delta.isexecutorclosure
Delta.isourclosure = Delta.isexecutorclosure
...
function Delta.isrbxactive()
	return windowActive
end
Delta.isgameactive = Delta.isrbxactive
Delta.iswindowactive = Delta.isrbxactive

function Delta.getinstances()
	return _game:GetDescendants()
end

local nilinstances, cache = {Instance.new("Part")}, {cached = {}}

function Delta.getnilinstances()
	return nilinstances
end
...
function Delta.getrunningscripts()
	local scripts = {}
	for _, v in pairs(Delta.getinstances()) do
		if v:IsA("LocalScript") and v.Enabled then table.insert(scripts, v) end
	end
	return scripts
end
Delta.getscripts = Delta.getrunningscripts

function Delta.getloadedmodules()
	local modules = {}
	for _, v in pairs(Delta.getinstances()) do
...
function Delta.checkcaller()
	local info = debug.info(Delta.getgenv, 'slnaf')
	return debug.info(1, 'slnaf')==info
end

function Delta.getthreadcontext()
	return 3
end
Delta.getthreadidentity = Delta.getthreadcontext
Delta.getidentity = Delta.getthreadcontext

function Delta.setthreadidentity()
	return 3, "Not Implemented"
end
Delta.setidentity = Delta.setthreadidentity
Delta.setthreadcontext = Delta.setthreadidentity

function Delta.getsenv(script_instance)
...
function Delta.getscripthash(instance) -- !
...
function Delta.hookfunction(func, rep)
	local env = getfenv(debug.info(2, 'f'))
...
Delta.replaceclosure = Delta.hookfunction

function Delta.cloneref(reference)
...
function Delta.compareinstances(x, y)
...
function Delta.gethui()
	return Delta.cloneref(_game:FindService("CoreGui"))
end

function Delta.isnetworkowner(part)
...
function Delta.deepclone(object) -- used for initialization
	local lookup = {}
...
Delta.debug = table.clone(debug) -- the debug funcs was not by me (.risazaza) credits goes to the person that made it
function Delta.debug.getinfo(f, options)
...
function Delta.debug.getmetatable(table_or_userdata)
...
Delta.debug.setmetatable = setmetatable

function Delta.getrawmetatable(object)
	...
	local raw_mt = Delta.debug.getmetatable(object)
	if raw_mt and raw_mt.__metatable then
		raw_mt.__metatable = nil 
		local result_mt = Delta.debug.getmetatable(object)
...
function Delta.setrawmetatable(object, newmetatbl)
...
function Delta.hookmetamethod(t, index, func)
	...
	local o = t
	local mt = Delta.debug.getmetatable(t)
	mt[index] = func
	t = mt
	return o
end
...
function Delta.getfpscap()
	return fpscap
end

function Delta.mouse1click(x, y)
	x = x or 0
	y = y or 0
...
function Delta.mouse1press(x, y)
	x = x or 0
	y = y or 0
...
function Delta.mouse1release(x, y)
	x = x or 0
	y = y or 0
...
function Delta.mouse2click(x, y)
	x = x or 0
	y = y or 0
...
function Delta.mouse2press(x, y)
	x = x or 0
	y = y or 0
...
function Delta.mouse2release(x, y)
	x = x or 0
	y = y or 0
...
function Delta.mousemoveabs(x, y)
	x = x or 0
	y = y or 0
...
function Delta.getscriptclosure(s)
	return function()
		return table.clone(Delta.require(s))
	end
end
Delta.getscriptfunction = Delta.getscriptclosure

function Delta.isscriptable(object, property)
...
task.spawn(function() -- queue_on_teleport handler
	...
	if type(source) == "string" and source ~= "" then
		...
		setfenv(rawLoadstringFunc, merge(getfenv(rawLoadstringFunc), Delta))
...
local function listen(coreModule)
	while task.wait() do
		...
		if type(execution_table) == "table" and execution_table["x e n o"] and (not execution_table.__executed) and coreModule.Parent == scriptsContainer then
			...
			setfenv(execLoad, merge(getfenv(execLoad), Delta))
			task.spawn(execLoad)
